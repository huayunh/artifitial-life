...
# David Kosbie
EVENT_BASED_ANIMATION_VERSION = "1.10.2"

# change log
# 03-01-16 v1.10.2
#   * users are now given the access to Root() and an additional
#   *   option in run() of fullscr (boolean)
# 01-22-16 v1.10.1
#   * modified to adopt to the requirement of the experiment
...
def run(initFn=None, mouseFn=None, keyFn=None, stepFn=None, drawFn=None,
        ...
        keyReleaseFn=None, quitFn=None, drawErrFn = None,
        width=300, height=300, timerDelay=128, errorFn = None,
    ...
    def reset():
        ...
        data.isRunning = True
        data.errMsg = None
        data.userData.fullscr = False
        data.userData.modData = data
        data.isPaused = False
    ...
    def redrawErrMsg():
        ...
        msg = (linewrap(data.errMsg, 60) +
               "\n\nPress ctrl-r to reset your current progress and go back to the home page.\n")
        fontSize = 8
        while True:
            ...
            if ((textWidth > width*.8) or (textHeight > height*.8)):
                break
            fontSize += 2
        data.canvas.create_rectangle(0,0,width,height,
            width = 0, fill = "#AA4444")
        data.canvas.create_text(width/2, height/5,
                                text=msg, font=font, fill="gray",
    ...
    def redrawAll():
        ...
        try:
            try:
                ...
                if (data.errMsg):
                    if (drawErrFn != None):
                        drawErrFn(data.canvas, data.userData)
                    else:
                        redrawErrMsg()
                else:
    ...
    def doCommand(command):
        if (command == "go"): data.isPaused = False
            ...
            doStep()
            # print "Calling onTimerFiredWrapper..."
            # onTimerFiredWrapper()  # DK: instead of doStep, so we get the timer loop running again
    ...
    def runAnimation():
        ...
        if data.userData.fullscr == True:
            data.root.attributes('-fullscreen', True)
        onTimerFiredWrapper()
        data.root.mainloop()
...
class Animation(object):
    ...
    def onDrawError(self,canvas):pass
    def onError(self):pass
    ...
    def run(self):
        ...
        def errorFn(): self.onError()
        def drawErrFn(canvas, data): self.onDrawError(canvas)
        run(initFn=initFn, mouseFn=mouseFn, keyFn=keyFn, stepFn=stepFn,
            ...
            quitFn=quitFn, drawErrFn = drawErrFn, errorFn = errorFn,
